...
import DungeonGeneration.Point;
...
public class AStar{
    ...
    public static int distance (boolean[][] passable, int startX, int startY, int goalX, int goalY, ArrayList<Point> path) {
        ...
        Node initial = new Node (startX, startY, 0, euclideanDistance (startX, startY, goalX, goalY), null);
        openSet.add (initial);
        while (true) {
            if (openSet.isEmpty()) {
    ...
    public static void BuildPath(Node lastNode, ArrayList<Point> path) {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(distance(testmap, 0, 0, 5, 5, path));
    }
    ...
    private static class Node implements Comparable {
        ...
        public Node (int x, int y, int costToGetHere, int estimatedCostToGoal, Node previous) {
            this.x = x;
            this.y = y;
        ...
        private void createAndAdd (int newX, int newY, int goalX, int goalY, boolean[][] passable,
            ...
            if (exists (newX, newY, passable) && passable[newX][newY]) {
                ...
                Node newNode = new Node (newX, newY, newCost, newEstimate, this);
